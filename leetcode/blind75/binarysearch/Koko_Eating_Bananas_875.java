...
//    }
     ...
     * !! 這題重要的是邊界處理
     * 正常 start 大家都會想成是 1, end 大家都會想成是裡面最大的 piles 裡面最大的
     * 但這樣就會放大了範圍
    ...
    class Solution {private int count = 0;
        public int minEatingSpeed(int[] piles, int h) {
            ...
            int start = (int) ceilDiv(sum, h);
            int end = (int) ceilDiv(sum, h - n + 1);
            int mid = 1;
            while (start <= end) {
                count++;
                mid = (start + end) / 2;
                int time = 0;
                // 這段是去看個時間是否超過上限 h ，如果超過就跳出不用繼續往下算
                for (int i = 0; i < n; i++) {
                    count++;
